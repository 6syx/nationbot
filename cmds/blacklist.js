...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (arid.toLowerCase() == 'add') {
            if (config.immigration.settings.blacklistedgroups.find(element => element == usera)) return interaction.reply('This group is already on the blacklist.')
            const gObj = await roblox.getGroup(Number(gu)).catch(err => {
                console.log(err)
                return interaction.reply("An error occured while getting the group.")
            })
            config.immigration.settings.blacklistedgroups.push(Number(gu))
            fs.writeFileSync('./config.json', JSON.stringify(config, null, 4))
            return interaction.reply(`${gObj.name} (${gu}) has been added to the immigration blacklist.`)
            ...
            fs.writeFileSync('./config.json', JSON.stringify(config, null, 4))
            return interaction.reply(`${gObj.name} (${gu}) has been removed from the immigration blacklist if they were on it.`)
            ...
            const iEmbed = new discord.MessageEmbed()
                ...
                .setColor('BLUE')
                .setTimestamp()
            return interaction.reply(iEmbed)
        } else {
            return interaction.reply(`Second argument must be either \`add\`, \`remove\`, or \`view\`.`)
        }
        ...
        if (arid.toLowerCase() == 'add') {
            ...
            if (config.immigration.settings.blacklistedusers.find(element => element == uID)) {
                return interaction.reply('This user is already on the blacklist.')
            }
            let realname = await roblox.getUsernameFromId(uID)
            config.immigration.settings.blacklistedusers.push(Number(uID))
            fs.writeFileSync('./config.json', JSON.stringify(config, null, 4))
            return interaction.reply(`${realname} (${uID}) has been added to the immigration blacklist.`)
        } else if (arid.toLowerCase() == 'remove') {
                let uID = await roblox.getIdFromUsername(gu).catch(err => {
                console.log(err)
                return interaction.reply(`An error occured.`)
            ...
            fs.writeFileSync('./config.json', JSON.stringify(config, null, 4))
            return interaction.reply(`${realname} (${uID}) has been removed from the immigration blacklist if they were on it.`)
        } else if (arid.toLowerCase() == 'view') {
            ...
            return interaction.reply(`First argument must be either \`groups\` or \`users\`.`)
        }
    }
}